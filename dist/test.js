riot.tag2('sample', '<h3>{message}</h3> <nav> <table if="{docs.status}"> <th>Nama</th> <th>Kelas</th> <th>Create At</th> <th>Update At</th> <tr each="{doc in docs.data}"> <td>{doc.title}</td> <td>{doc.content}</td> <td>{doc.created_at}</td> <td>{doc.updated_at}</td> </tr> </table> </nav>', 'sample table,[data-is="sample"] table{ font-family: arial, sans-serif; border-collapse: collapse; width: 100%; } sample td,[data-is="sample"] td,sample th,[data-is="sample"] th{ border: 1px solid #dddddd; text-align: left; } sample tr:nth-child(even),[data-is="sample"] tr:nth-child(even){ background-color: #dddddd; } sample,[data-is="sample"]{ font-size: 2rem } sample h3,[data-is="sample"] h3{ color: #444 } sample ul,[data-is="sample"] ul{ color: #999 }', '', function(opts) {
"use strict";
